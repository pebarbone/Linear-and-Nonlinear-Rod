...
Next, let's consider the same nonlinear elastic rod solved with two finite elements. The weak form can be restated from (ref{nonlin_weak}):
...
\beq
\int_0^L \left[
E \frac{du^*}{dx} e^{\beta \frac{du^*}{dx}} \frac{dv}{dx} - f(x)v(x)
\right] dx - \sigo v(L) = 0
\eeq
}
\end{tcolorbox}
